...
def get_store_data(driver:WebDriver, scroll_container: WebElement, file: TextIOWrapper):
  ...
  for index in range(len(get_store_li)):
    ...
    try:
      ...
      # BeautifulSoup을 이용하여 html파싱 후 텍스트 반환
      ...
      # 다음 매장으로 이동하기 위해 검색결과 Iframe으로 전환
      to_search_iframe(driver)
    except TimeoutException:
      to_search_iframe(driver)
```
...
```
name|address
자마버거 이대점|서울 서대문구 연세로2마길 17 1층
헤이죠지|서울 마포구 동교로46길 42-24 1층 헤이죠지
카쿠시타|서울 마포구 동교로38길 33-15
정육면체|서울 서대문구 연세로5다길 22-8 1층 정육면체
하늘초밥|서울 서대문구 이화여대2길 14 1층 하늘초밥
시오|서울 서대문구 연희로11가길 23 1층
홍대IN파스타|서울 마포구 홍익로5안길 32 준빌라
미쁘동|서울 마포구 동교로38길 33-21 2층
담산 신촌본점|서울 서대문구 연세로5다길 5 1층 담산 신촌본점
```
